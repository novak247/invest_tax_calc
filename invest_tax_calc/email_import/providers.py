...
class GmailClient:
    ...
    def iter_report_attachments(
        ...
        *,
        ...
        for message_id in self.iter_message_ids(query=query, max_messages=max_messages):
            yield from self.iter_message_attachments(message_id)

    def iter_message_ids(self, *, query: str, max_messages: int) -> Iterator[str]:
        yield from self._iter_message_ids(query=query, max_messages=max_messages)

    def iter_message_attachments(self, message_id: str) -> Iterator[Attachment]:
